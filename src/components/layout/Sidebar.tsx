...
export function Sidebar({ defaultCollapsed = false }: SidebarProps) {
  ...
  return (
    <aside
      className={cn(
        "hidden lg:flex flex-col border-r border-border bg-surface transition-all duration-200 shrink-0 overflow-hidden",
    ...
    >
      ...
      >
        {collapsed ? (
          ...
          <>
            <Link href="/app/dashboard" className="flex items-center gap-2.5 flex-1 min-w-0">
              <Image src="/vercel.svg" alt="Logo" width={16} height={16} className="dark:invert shrink-0 ml-2" />
      ...
      <nav className="flex-1 overflow-y-auto overflow-x-hidden p-2 flex flex-col gap-0.5">
        {items.map((item, i) => {
          ...
          const showGroup = !collapsed && item.group && items[i - 1]?.group !== item.group
          return (
            <div key={item.href}>
              {showGroup && (
                <p className="px-2.5 pt-3 pb-1 text-xs font-medium text-fg-3 first:pt-1">
                  {item.group}
                </p>
              )}
              <Link
